...
public class LookingAtPointCtrl : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!followAss.FPSMode())
        {
            ...
            //WS
            float DeltaHeight=0;
            if ((Input.GetKey(KeyCode.W) || Input.GetAxis("Vertical") > 0.1f) && transform.localPosition.y < HeightMax)
            {
                DeltaHeight += XRotateSpeed;
            }
            else if (Input.GetKey(KeyCode.S) || Input.GetAxis("Vertical") < -0.1f && transform.localPosition.y > HeightMin)
            {
                DeltaHeight -= XRotateSpeed;
            }
